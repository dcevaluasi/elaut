...
const VerifikatorName = ({ id }: { id: string | number }) => {
    ...
    if (loading) return (
        <div className="flex items-center gap-1.5 mt-2 px-2.5 py-1 bg-blue-500/5 rounded-full border border-blue-500/10 animate-pulse">
            <div className="w-1.5 h-1.5 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]" />
            <span className="text-[9px] font-bold text-blue-500/60 uppercase tracking-tighter">Syncing Verifier</span>
        </div>
    );
    ...
    return (
        <div className="group/verif relative flex items-center gap-2 mt-2 px-2.5 py-1 bg-slate-950/[0.03] dark:bg-white/[0.03] rounded-full border border-slate-200/50 dark:border-white/5 transition-all hover:bg-white dark:hover:bg-white/10 hover:shadow-xl hover:shadow-blue-500/5">
            <div className="flex -space-x-1">
                <div className="w-5 h-5 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-400 flex items-center justify-center text-[8px] font-black text-white shadow-sm ring-2 ring-white dark:ring-slate-900">
                    {adminPusatData.Nama.charAt(0).toUpperCase()}
                </div>
            </div>
            <div className="flex flex-col">
                <span className="text-[8px] font-black text-blue-500/70 uppercase tracking-[0.1em] leading-none">Internal Verifier</span>
                <span className="text-[10px] font-extrabold text-slate-700 dark:text-slate-300 truncate max-w-[100px] leading-tight mt-0.5">
                    {adminPusatData.Nama}
                </span>
            </div>
            <RiVerifiedBadgeFill className="text-blue-500 w-3 h-3 ml-auto animate-in zoom-in duration-500" />
        </div >
    );
};
    ...
    return (
        <div className="space-y-8">
            <div className="relative group/table container-fluid">
                ...
                <div className="relative bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl rounded-[40px] border border-white/40 dark:border-white/5 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)] dark:shadow-none overflow-hidden">
                    <div className="overflow-x-auto scrollbar-hide">
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="border-b border-black/[0.03] dark:border-white/[0.03] bg-black/[0.01] dark:bg-white/[0.01]">
                                    <th className="px-8 py-7 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 text-center w-20">UID</th>
                                    <th className="px-8 py-7 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 text-left">Pelatihan / Institusi</th>
                                    <th className="px-8 py-7 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 text-center">Metrik & Klasifikasi</th>
                                    <th className="px-8 py-7 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 text-center">Verifikator</th>
                                    <th className="px-8 py-7 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 text-center">Status</th>
                                    <th className="px-8 py-7 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 text-right w-44">Aksi</th>
                                </tr>
                            </thead>
                                    ...
                                    {paginatedData.map((pelatihan: PelatihanMasyarakat, index: number) => {
                                        ...
                                        return (
                                            <motion.tr
                                                ...
                                                className="group/row relative hover:bg-white/60 dark:hover:bg-white/[0.03] transition-all duration-500 border-l-4 border-l-transparent hover:border-l-blue-500"
                                            >
                                                <td className="px-8 py-10 text-center">
                                                    <div className="relative inline-flex items-center justify-center">
                                                        <div className="absolute inset-0 bg-blue-500/10 rounded-full scale-0 group-hover/row:scale-150 transition-transform duration-700 opacity-0 group-hover/row:opacity-100" />
                                                        <span className="relative text-[11px] font-black text-slate-300 dark:text-slate-700 group-hover/row:text-blue-600 transition-colors">
                                                ...
                                                <td className="px-8 py-10 text-center">
                                                    <div className="inline-flex flex-col items-center min-w-[140px]">
                                                        {pelatihan.VerifikatorPelatihan ? (
                                                            ...
                                                            <div className="flex flex-col items-center gap-1.5 opacity-30 grayscale hover:grayscale-0 hover:opacity-100 transition-all cursor-help">
                                                                ...
                                                                <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Unassigned</span>
                                                            </div>
                                                        )}
                                                    </div>
                                                </td>

                                                <td className="px-8 py-10 text-center min-w-[220px]">
                                                    <div className="flex flex-col items-center gap-2">
                                                        <div className={`
                                                            group/status relative flex items-center gap-3 px-5 py-2.5 rounded-2xl shadow-sm border
                                                            ${color} transition-all duration-500 group-hover/row:-translate-y-1 group-hover/row:shadow-xl
                                                        `}>
                                                            <div className="absolute inset-x-0 bottom-0 h-1 bg-black/10 dark:bg-white/10 origin-left scale-x-0 group-hover/status:scale-x-100 transition-transform duration-1000" />
                                                            <span className="animate-in zoom-in spin-in-12 duration-1000">{icon}</span>
                                                            <span className="text-[10px] font-black uppercase tracking-[0.15em]">{label}</span>
                                                            <div className="ml-2 w-1.5 h-1.5 rounded-full bg-current animate-pulse shadow-[0_0_8px_currentColor]" />
                                                        </div>
                                                    </div>
                                                </td>
                                                    ...
                                                    <div className="flex flex-col gap-2.5 items-end">
                                                        {(Cookies.get('Access')?.includes('isSigning') || Cookies.get('Access')?.includes('verifyPelaksanaan') || Cookies.get('Access')?.includes('supervisePelaksanaan')) && (
                                                            <Link
                                                                ...
                                                                className="group/btn relative overflow-hidden flex items-center gap-2.5 px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-500 text-white rounded-[18px] text-[11px] font-black shadow-lg shadow-blue-500/25 hover:shadow-blue-500/40 hover:-translate-y-1 active:translate-y-0 transition-all duration-300"
                                                            >
                                                                <div className="absolute inset-y-0 left-0 w-1 bg-white/20 origin-bottom scale-y-0 group-hover/btn:scale-y-100 transition-transform duration-500" />
                                                                <span className="tracking-[0.1em]">DETAIL REVIEW</span>
                                                                <ArrowUpRight size={14} className="group-hover/btn:rotate-45 transition-transform duration-500 ease-out" />
                                                            </Link>
                                                        )}
                                                        ...
                                                        {Cookies.get('Access')?.includes('createPelatihan') && (
                                                            <Link
                                                                href={`/admin/${usePathname().includes('pusat') ? 'pusat' : 'lemdiklat'}/pelatihan/detail/${pelatihan.KodePelatihan}/${encryptValue(pelatihan.IdPelatihan.toString())}`}
                                                                className="group/btn relative overflow-hidden flex items-center gap-2.5 px-6 py-3 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-[18px] text-[11px] font-black shadow-lg hover:-translate-y-1 active:translate-y-0 transition-all duration-300"
                                                            >
                                                                <Settings size={14} className="group-hover/btn:rotate-180 transition-transform duration-700" />
                                                                <span className="tracking-[0.1em]">Manage</span>
                                                            </Link>
                                                        )}
                                                    </div>
                                                </td>
