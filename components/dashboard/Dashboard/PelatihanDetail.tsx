...
const PelatihanDetail: React.FC<Props> = ({ data, fetchData }) => {
    ...
    const DetailCard = ({ label, value, icon: Icon, theme = "blue" }: { label: string; value?: string; icon: any; theme?: string }) => {
        const themes: any = {
            blue: "from-blue-50 to-indigo-50 text-blue-600 border-blue-100/50",
            amber: "from-amber-50 to-orange-50 text-amber-600 border-amber-100/50",
            emerald: "from-emerald-50 to-teal-50 text-emerald-600 border-emerald-100/50",
            rose: "from-rose-50 to-pink-50 text-rose-600 border-rose-100/50",
            indigo: "from-indigo-50 to-violet-50 text-indigo-600 border-indigo-100/50",
        ...
        return (
            ...
            >
                <div className="flex items-center gap-2.5 mb-3">
                    <div className="p-2.5 rounded-2xl bg-white border border-slate-100 shadow-sm transition-transform group-hover:scale-110 group-hover:rotate-6">
                        <Icon className="w-4 h-4" />
                    </div>
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] leading-none">{label}</span>
                </div>
                {value?.includes('</p>') ? (
                    <div className="prose prose-sm text-slate-700 leading-relaxed max-w-none font-bold">
                        <div dangerouslySetInnerHTML={{ __html: value }} />
                    </div>
                ) : (
                    <span className="text-sm font-black text-slate-900 line-clamp-2 tracking-tight">
                        {value || "-"}
                    </span>
                )}
            </motion.div>
    ...
    return (
        <div className="w-full space-y-4 md:space-y-6 py-2">
            ...
            <div className="relative group">
                <div className="absolute inset-0 bg-blue-600/5 rounded-[3rem] blur-2xl group-hover:blur-3xl transition-all duration-700" />
                <div className="bg-white/60 backdrop-blur-2xl border border-white rounded-[3rem] overflow-hidden shadow-2xl relative">
                    <div className="px-6 md:px-8 py-4 md:py-5 border-b border-slate-100 bg-slate-50/30 flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="font-black text-lg md:text-xl text-slate-900 tracking-tight">Pusat Kontrol Pelaksanaan</h3>
                                <p className="text-[10px] md:text-xs text-slate-500 font-medium">Kelola status, verifikasi, dan alur kerja pelaksanaan pelatihan</p>
                            </div>
                        </div>
                                    ...
                                    <>
                                        ...
                                        {data.SuratPemberitahuan !== "" && (data.StatusPenerbitan === "0" || data.StatusPenerbitan === "1.2") ? (
                                            data?.UserPelatihan?.length !== 0 && data?.SuratPemberitahuan !== "" ? (
                                                ...
                                                <Button
                                                    variant="outline"
                                                    className="h-12 flex items-center gap-3 rounded-[1.25rem] px-8 shadow-sm transition-all border-blue-200 text-blue-600 font-black uppercase tracking-wider text-xs hover:bg-blue-50"
                                                    onClick={() => {
                                                        Toast.fire({
                                                            icon: "warning",
                                                            title: "Lengkapi Data",
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center gap-4">
                                <div className="px-3 py-1 bg-slate-100 border border-slate-200 rounded-lg">
                                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Status & Verifikasi</span>
                                </div>
                                <div className="h-px flex-1 bg-gradient-to-r from-slate-100 to-transparent"></div>
                            </div>

                            {
                                data.SuratPemberitahuan == "" ?
                                    <div className="py-10 w-full bg-slate-50/50 rounded-[2.5rem] border border-dashed border-slate-200 flex items-center flex-col justify-center gap-4 group">
                                        <div className="relative">
                                            <div className="absolute inset-0 bg-slate-400/20 blur-2xl rounded-full scale-150 animate-pulse" />
                                            <div className="w-16 h-16 rounded-[1.5rem] bg-white flex items-center justify-center shadow-xl border border-slate-100 relative z-10">
                                                <Lock className='w-8 h-8 text-slate-300' />
                                            </div>
                                        </div>
                                        <div className="text-center space-y-1 max-w-md">
                                            <h4 className="font-black text-slate-800 uppercase tracking-tight">Menunggu Administrasi</h4>
                                    ...
                                    <div className={`grid grid-cols-1 sm:grid-cols-2 ${adminPusatData != null ? 'lg:grid-cols-3' : ''} gap-4`}>
                                        ...
                                        <motion.div
                                            whileHover={{ scale: 1.02 }}
                                            className="flex flex-col p-6 bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 group"
                                        >
                                            <span className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 group-hover:text-blue-600 transition-colors">Surat Pemberitahuan</span>
                                            <Link target="_blank" href={`${urlFileSuratPemberitahuan}/${data?.SuratPemberitahuan}`} className="text-base font-black text-blue-600 truncate flex items-center gap-2 hover:translate-x-1 transition-transform">
...

                                        {
                                            adminPusatData != null && (
                                                <motion.div
                                                    ...
                                                    className="flex flex-col p-6 bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 group"
                                                >
                                                    <span className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 group-hover:text-amber-600 transition-colors">Verifikator Pusat</span>
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-10 h-10 rounded-full bg-amber-50 border border-amber-100 flex items-center justify-center text-amber-600 font-black">
            ...
            >
                ...
                <AccordionSection value="pins" title="Informasi Umum Pelatihan" icon={<TbSchool className="text-blue-600" />} description="Kelola data dasar pelatihan, sumber pembiayaan, dan detail teknis pelaksanaan.">
                    <div className="flex flex-col w-full gap-8">
                        <div className="flex items-center gap-3 bg-slate-50 p-2.5 rounded-2xl w-fit border border-slate-100">
                            <EditPelatihanAction
                                idPelatihan={data.IdPelatihan.toString()}
                                currentData={data}
                                onSuccess={fetchData} />
                            ...
                            {data?.FotoPelatihan != "https://elaut-bppsdm.kkp.go.id/api-elaut/public/static/pelatihan/" ? (
                                <div className="flex flex-col lg:flex-row gap-8">
                                    <div className="w-full lg:w-[350px] flex-shrink-0">
                                        <div className="relative group rounded-[2.5rem] overflow-hidden shadow-2xl border-[6px] border-white aspect-[3/4.5] bg-slate-100">
                                            <Image
                                    ...
                                    <div className="flex-1 flex flex-col gap-4">
                                        ...
                                        <div className="bg-white rounded-[2.5rem] p-6 border border-slate-100 shadow-inner h-full flex flex-col gap-3 group">
                                            <div className="flex items-center gap-3">
                                                <div className="p-2 rounded-xl bg-indigo-50 text-indigo-600">
                                                    <MdOutlineDescription className="w-5 h-5" />
                                                </div>
                                                <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Deskripsi Publikasi</span>
                                            </div>
                                            <div className="text-sm text-slate-700 leading-relaxed font-bold tracking-tight first-letter:text-3xl first-letter:font-black first-letter:text-blue-600 first-letter:mr-1">
                                ...
                                <div className="py-10 w-full bg-slate-50/50 rounded-[2.5rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center gap-4 group">
                                    <div className="relative">
                                        <div className="absolute inset-0 bg-indigo-500/10 blur-3xl rounded-full scale-150 animate-pulse" />
                                        <div className="w-20 h-20 rounded-[1.5rem] bg-white border border-slate-100 flex items-center justify-center text-slate-300 shadow-xl relative z-10 group-hover:rotate-6 transition-transform">
                                            <TbUserShare className="w-10 h-10" />
                                        </div>
                                    </div>
                                    <div className="text-center max-w-sm space-y-1">
                                        <h4 className="font-black text-slate-800 uppercase tracking-tight text-xl">Promosi Belum Tersedia</h4>
                    ...
                    <div className="flex flex-col w-full gap-4">
                        ...
                        {data.ModuleMateri == "" ? (
                            <div className="py-10 w-full bg-slate-50/50 rounded-[2.5rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center gap-4 group">
                                ...
                                <div className="text-center max-w-sm space-y-1">
                                    <h4 className="font-black text-slate-800 uppercase tracking-tight text-xl">Perangkat Belum Dipilih</h4>
                                    <p className="text-sm text-slate-500 font-medium leading-relaxed italic">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 gap-4">
                                    {modulPelatihan?.ModulPelatihan?.map((modul: ModulPelatihan, idx: number) => (
                                        <motion.div
                                            ...
                                            className={`group bg-white rounded-[2rem] border ${expanded === modul.IdModulPelatihan ? 'border-blue-200 shadow-2xl shadow-blue-500/5' : 'border-slate-100 shadow-sm'} p-6 transition-all duration-300 cursor-pointer hover:shadow-xl hover:shadow-slate-200/50`}
                                        >
                                            <div className="flex items-center justify-between gap-6">
                                                <div className="flex items-center gap-6">
                                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl transition-all duration-500 ${expanded === modul.IdModulPelatihan ? 'bg-blue-600 text-white shadow-xl shadow-blue-500/30 rotate-3' : 'bg-slate-50 text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-600'}`}>
                                                        <GraduationCap className="shrink-0 w-6 h-6" />
                                                    </div>
                                                    <div>
                                                        <h4 className={`font-black text-lg tracking-tight transition-colors ${expanded === modul.IdModulPelatihan ? 'text-blue-600' : 'text-slate-800 group-hover:text-blue-600'}`}>{modul.NamaModulPelatihan}</h4>
                                                        <div className="flex items-center gap-4 mt-2">
                                                            <div className="flex items-center gap-2">
                                                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">UNIT LOKASI:</span>
                                                                <Link href={`${process.env.NEXT_PUBLIC_MODULE_FILE_URL}/${modul.FileModule}`} target="_blank" className="text-xs font-black text-blue-600 hover:underline flex items-center gap-1.5 bg-blue-50 px-3 py-1 rounded-lg border border-blue-100">
                                                ...
                                                <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${expanded === modul.IdModulPelatihan ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-400'}`}>
                                            ...
                                            <AnimatePresence>
                                                {expanded === modul.IdModulPelatihan && (
                                                    ...
                                                    >
                                                        <div className="mt-8 pt-8 border-t border-slate-100 grid gap-8">
                                                            <div className="flex items-center gap-3">
                                                                <div className="p-1.5 rounded-lg bg-emerald-50 text-emerald-600">
                                                                    <TbRocket className="w-4 h-4" />
                                                                </div>
                                                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Bahan Penunjang & Media Belajar Digital</span>
                                                            </div>

                                                            {modul.BahanTayang.length === 0 ? (
                                                                <div className="bg-slate-50/50 rounded-[2rem] p-8 text-center border-2 border-dashed border-slate-100">
                                                                    <p className="text-xs text-slate-400 font-black uppercase tracking-widest italic tracking-tight">Belum ada bahan tayang tambahan</p>
                                                                </div>
                                                            ) : (
                                                                <div className="grid sm:grid-cols-2 gap-4">
                                                                    {modul.BahanTayang.map((row_bt, idx) => (
                                                                        <motion.div
                                                                            key={idx}
                                                                            whileHover={{ x: 5 }}
                                                                            className="flex items-center justify-between p-5 bg-white rounded-2xl border border-slate-100 hover:border-blue-200 hover:shadow-lg transition-all"
                                                                        >
                                                                            <div className="flex items-center gap-4">
                                                                                <div className="w-12 h-12 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 shadow-inner group-hover:bg-blue-50 group-hover:text-blue-600 transition-colors">
                                                                                    <FileText className="w-5 h-5" />
                                                                                </div>
                                                                                <div>
                                                                                    <p className="text-sm font-black text-slate-800 leading-tight">{row_bt.NamaBahanTayang}</p>
                                ...
                                <div className="bg-blue-50/50 p-8 rounded-[2.5rem] border border-blue-100 flex items-start gap-5 shadow-inner">
                                    ...
                                    </div>
                                    {
                                        Cookies.get('Role') == 'Pengelola UPT' && <div className="space-y-1">
                                            <p className="text-xs font-black text-blue-600 uppercase tracking-widest">Catatan Sinkronisasi</p>
                                            <p className="text-[11px] text-slate-600 leading-relaxed font-bold tracking-tight italic">
                                                Penambahan perangkat belajar baru dilakukan melalui menu <span className="text-blue-600 underline decoration-dotted underline-offset-4 cursor-pointer">MASTER MODUL</span>. Segala perubahan data media pada master akan otomatis terupdate secara realtime pada dashboard pelatihan ini.
                                            </p>
                                        </div>
                                    }
                    ...
                    <div className="flex flex-col w-full gap-4">
                        ...
                        {data?.Instruktur == "" ? (
                            <div className="py-10 w-full bg-slate-50/50 rounded-[2.5rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center gap-4 group">
                                ...
                                <div className="text-center max-w-sm space-y-1">
                                    <h4 className="font-black text-slate-800 uppercase tracking-tight text-xl">Instruktur Belum Mapping</h4>
                                    <p className="text-sm text-slate-500 font-medium leading-relaxed italic">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {instrukturs?.map((item, idx) => (
                                    <motion.div
                                        key={idx}
                                        whileHover={{ y: -8 }}
                                        className="relative group bg-white border border-slate-100 rounded-[3rem] p-8 shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden"
                                    >
                                        <div className="relative z-10 space-y-3">
                                            <div className="flex justify-between items-start">
                                                <div className="flex items-center gap-4">
                                                    <div className="relative">
                                                        ...
                                                        <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-emerald-500 rounded-full border-[3px] border-white z-20 flex items-center justify-center pointer-events-none">
                                                            <div className="w-1 h-1 bg-white rounded-full animate-ping" />
                                                        </div>
                                                    </div>
                                                    <div>
                                                        <h3 className="font-black text-slate-900 text-xl tracking-tight group-hover:text-blue-600 transition-colors leading-tight">{item.nama}</h3>
                                                        <div className="inline-flex items-center gap-2 mt-2 px-3 py-1 bg-blue-50 rounded-lg border border-blue-100">
                                                            <TbCertificate className="w-3.5 h-3.5 text-blue-600" />
                                                            <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest leading-none">{item.bidang_keahlian || "AHLI MATERI"}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="bg-slate-50 p-4 rounded-[1.5rem] border border-slate-100 shadow-inner group/data">
                                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1.5 group-hover/data:text-blue-500 transition-colors">IDENTITAS NIP</p>
                                                    <p className="text-sm font-black text-slate-800 tracking-tight">{item.nip}</p>
                                                </div>
                                                <div className="bg-slate-50 p-4 rounded-[1.5rem] border border-slate-100 shadow-inner group/data">
                                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1.5 group-hover/data:text-blue-500 transition-colors">KONTAK RESMI</p>
                                                    <p className="text-sm font-black text-slate-800 tracking-tight">{item.no_telpon}</p>
                                                </div>
                                            </div>

                                            <div className="p-6 bg-gradient-to-br from-slate-50 to-slate-100 rounded-[2rem] border border-slate-100 shadow-inner group/meta">
                                                <div className="flex items-center gap-3 mb-3">
                                                    <div className="p-1.5 rounded-lg bg-white text-slate-400 shadow-sm">
                                                        <TbBuildingSkyscraper className="w-4 h-4" />
                                                    </div>
                                                    <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Penugasan & Unit Kerja</p>
                                                </div>
                                                <p className="text-sm font-black text-slate-800 leading-relaxed tracking-tight">{item.jenjang_jabatan}</p>
                                                <div className="mt-2 flex flex-col gap-1">
                                                    <p className="text-[11px] text-slate-500 font-bold uppercase tracking-tight">{item.eselon_1}</p>
                                                    <p className="text-[10px] text-slate-400 font-medium italic truncate">{item.eselon_2}</p>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <AccordionSection value="peserta" title="Manajemen Peserta Pelatihan" icon={<TbUsers className="text-rose-600" />} description="Daftar peserta yang terdaftar dalam pelatihan. Lakukan validasi data atau import peserta baru di sini.">
                        <div className="flex flex-col w-full gap-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <motion.div
                                    whileHover={{ scale: 1.02 }}
                                    className="group relative overflow-hidden bg-white border border-slate-100 p-6 rounded-[2.5rem] shadow-xl shadow-slate-200/50 transition-all duration-500"
                                >
                                    <div className="relative z-10 flex items-center justify-between">
                                        <div className="space-y-1">
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-2 leading-none">Total Peserta Terdaftar</p>
                                            <div className="flex items-baseline gap-2">
                                                <p className="text-5xl font-black text-slate-900 tabular-nums tracking-tighter">{data?.UserPelatihan?.length || 0}</p>
                                                <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Orang</span>
                                            </div>
                                        </div>
                                        <div className="w-16 h-16 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center text-2xl border border-blue-100 shadow-xl shadow-blue-500/10 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500">
                                ...
                                >
                                    <div className="relative z-10 flex items-center justify-between">
                                        <div className="space-y-1">
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-2 leading-none">Status Validasi System</p>
                                            <div className="flex items-baseline gap-2">
                                        ...
                                        <div className="w-16 h-16 rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center text-2xl border border-emerald-100 shadow-xl shadow-emerald-500/10 group-hover:scale-110 group-hover:-rotate-6 transition-all duration-500">
                            ...
                            <div className="relative group/table mt-4 bg-white/40 backdrop-blur-2xl rounded-[2.5rem] border border-slate-100 overflow-hidden shadow-2xl transition-all duration-500 hover:shadow-blue-500/5">
