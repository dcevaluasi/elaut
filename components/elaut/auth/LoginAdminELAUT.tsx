...
export const LoginAdminELAUT = () => {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        if ((email.includes('politeknik') || email.includes('supm')) && role !== 'satdik') {
            Toast.fire({
                icon: "error",
                title: "Oops.",
        ...
        try {
            const response: AxiosResponse = await axios.post(
                `${baseUrl}/${role === 'balai' || role === 'satdik' ? 'lemdik' : 'adminPusat'}/login`,
            ...
            Cookies.set("XSRF093", role === "balai" ? "balai" : role == 'adminPusat' ? "adminPusat" : 'satdik', { expires: 1 });
                                ...
                                <SelectTrigger className="w-full rounded-xl p-4 h-[2.7rem] border border-blue-500">
                                    <p className="mr-3 flex items-center gap-1 text-base text-gray-300">
                                        ...
                                        {role != ""
                                            ? role == "adminPusat"
                                                ? "Admin Pusat"
                                                : role == 'balai' ? "Admin Balai Pelatihan" : 'Admin Satuan Pendidikan'
                                            : "Pilih Role"}
                                    </p>
                                </SelectTrigger>
